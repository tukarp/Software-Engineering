...
long double BernouliDiagram::bernouliDiagramRecursively(double p, int n, int k) {
    q = (1 - p);
    return (
        binomialTheoremPtr->binomialTheoremRecursively(n, k) *
        powerPtr->powerRecursively(p, k) *
...
long double BernouliDiagram::bernouliDiagramIteratively(double p, int n, int k) {
    q = (1 - p);
    return (
        binomialTheoremPtr->binomialTheoremIteratively(n, k) *
        powerPtr->powerIteratively(p, k) *
